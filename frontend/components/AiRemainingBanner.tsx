...
export default function AiRemainingBanner() {
  ...
  return (
    <div
      role="alert"
      className="sticky top-16 z-40 bg-gold/10 border-b border-gold/30 px-4 py-2"
    >
      <div className="max-w-content mx-auto flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm">
          <svg className="w-4 h-4 text-gold-dark flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <span className="text-gold-dark font-sans font-semibold uppercase tracking-wide text-xs">
            {t("remaining", { count: remaining })}
          </span>
        </div>
        <button
          onClick={dismiss}
          className="text-gold-dark/60 hover:text-gold-dark transition-colors flex-shrink-0"
          aria-label="Dismiss"
        >
